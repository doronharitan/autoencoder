...
The second part studies the neural representation of the autoencoder
and how it develops along the training.

Grayscale images (50X50 pixels) were used as the dataset. The images describe a rat running in a 
round arena with 6 ports around the walls. Each frame is labeled with multiple features:
 Location of the rat, the direction of its torso and its distance to the center of the arena.

# tl;dr
1. Convolutional autoencoder successfully denoises grayscale images.
2. The neuronal representation of the model encodes the structure of the arena 
and the features of the rat.
3. The development of the neural representation can be studies by the UMAP dimensionality reduction algorithm.
 
   The figure below shows how the 2D UMAP of the neural representation changes with training
...
## Table of content:
- Installation instructions and technical information how to run the scripts can be found in the file ['Installation_Readme.md'](https://github.com/doronharitan/autoencoder/blob/master/Installation_Readme.md)
...
- [The neuronal representation of the model encodes the structure 
...
- [Replacing the encoder part by other non-convolutional dimensionality reductions](#replacing-the-encoder-part-by-non-convolutional-dimensionality-reduction-techniques) of my project
...
Note: the dataset noise comes from the image acquisition system and not syntactically introduced into the dataset.

### The latent space encodes the structure of the arena and the features of the rat.
...
e.g. the body angle of the rat (Fig A). If the points are cluster by color or arranged in a color gradient it suggests 
...
The features I colored were: 
 ...
 2. The distance of the rat from the arena center (Fig B).
  ...
  The 2D point cloud is shaped in a circular structure with 6 elements that comes out form the circle. This spatial
   arrangements correlate with the arena geometry. 
   
   The color gradients and cluster in the plots suggest that the distance of the rat form the center of the arena and 
...
The changed architecture contains two parts:
...
For each technique I embedded all the images in 16D or 2D.
 ...
 latent space representation resembled the AE encoder latent space. The 
...
Its input is a single image of a rat and its output is the rat direction. 
...
The geometry of the 2D AE embedding (fig A) is significantly different than the direction tagger
 embedding (fig B). 
This can be explained by the difference in the objective functions: 
