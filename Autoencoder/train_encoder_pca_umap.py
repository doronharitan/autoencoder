...
from utils_local import setting_parameters, \
    ...
    get_latent_space_pca, get_latent_space_umap
...
def main():
    ...
    # ===== make sure that the save_latnt_space will not be True =====
    ...
    if args['dim_reduction_algo'] == 'PCA':
        dataset_dict = get_latent_space_pca(args, folder_dir)
    else:
        dataset_dict = get_latent_space_umap(args, folder_dir)
    dataloader_dict = {index: DataLoader(data,
                                             batch_size=args['batch_size'],
                                             shuffle=True)
                           for index, data in dataset_dict.items()}
    # ====== visualize validation data =======
    img_to_plot = dataloader_dict[1].dataset.tensors[1][:16]
    ...
    for epoch in range(args['epochs']):
        ...
        if (epoch % args['val_check_interval']) == 0:
            val_loss = test_model_with_labels(model, dataloader_dict[1], criterion, epoch, save_images_path)
            scheduler.step()
