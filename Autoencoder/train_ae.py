...
from utils_local import setting_parameters, loading_plus_preprocessing_data, \
    train_model, test_model, save_loss_info_into_a_file, create_folder_if_needed
import torch
...
def main():
    ...
    if args['save_latent_space']:
        dataloader_dict['all_data'] = loading_plus_preprocessing_data(args)

    # ====== visualize validation data =======
    img_to_plot = dataloader_dict[1].dataset[:16]
    ...
    for epoch in range(args['epochs']):
        ...
        train_loss = train_model(model, dataloader_dict, optimizer, criterion, epoch,
                                 args['save_latent_space'], folder_dir, args['checkpoint_latent_space_interval'])
        if (epoch % args['val_check_interval']) == 0:
            val_loss = test_model(model, dataloader_dict[1], criterion, epoch, save_images_path)
            scheduler.step()
            ...
            print('%d Epoch: Train Loss %.5f, Validation loss %.5f, time %.2f, lr %.8f'
